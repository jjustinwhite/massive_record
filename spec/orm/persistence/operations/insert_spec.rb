...
describe MassiveRecord::ORM::Persistence::Operations::Insert do
  ...
  describe "#execute" do
    ...
    it "raises a RecordNotUnique error if we should check it" do
      expect(record.class).to receive(:check_record_uniqueness_on_create).and_return true 
      expect(record.class).to receive(:exists?).with(record.id).and_return true
    ...
    it "calls upon store_record_to_database for help with actually insert job" do
      expect(subject).to receive(:store_record_to_database).with('create')
      subject.execute
    end
  end
end
